...
##########################################################################
def metrics(mask_root, pred_root):
    global FM
    global WFM
    global SM
    global EM
    ...
    # root = os.getcwd()
    # mask_root = "/home/fzq/Desktop/Dataset/MAS3K/test/Mask/"
    # pred_root = os.path.join(root, "../eval_results/MAS/") 
    mask_name_list = sorted(os.listdir(mask_root))
    for mask_name in tqdm(mask_name_list, total=len(mask_name_list), ncols=100):
        
        # mask_path = os.path.join(mask_root, pred_name)
        # pred_path = os.path.join(pred_root, pred_name)
        
        mask_name = mask_name.split('.')[0]
        mask_path = os.path.join(mask_root, mask_name + '.png') # mas3k=png, rmas=jpg
        pred_path = os.path.join(pred_root, mask_name + '.png')
    ...
    with open("../eval_results/result.txt", "w") as result_file:
        result_file.write(str(results))


if __name__ == '__main__':
    mask_root = '/home/fzq/dataset/MAS3K/test/Mask/'
    pred_root = '../eval_results/MAS/'
    
    metrics(mask_root, pred_root)
