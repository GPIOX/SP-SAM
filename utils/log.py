...
def get_logger(exp_name, verbosity=1, name=None):
    ...
    path = os.path.join('runs', exp_name, 'logs')
    os.makedirs(path, exist_ok=True)
    
    log_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    log_name = os.path.join(path, f'{log_time}.log')
    fh = logging.FileHandler(log_name, "w") #输出到日志文件
    # fh = logging.FileHandler("runs/logs/log_20240918_CUDA_1_UFO120.log", "w") #输出到日志文件
...
def save_ckpt(ckpt,**kwargs):
    torch.save(ckpt,os.path.join("best/", f"{kwargs['name']}", f"epoch_{kwargs['epoch']}.pth"))
