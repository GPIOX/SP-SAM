...
from segment_anything.segment_anything.tiny_vit_sam import TinyViTBlock

class LoRA_mobilesam_qkv(nn.Module):
    """
    ...
    def __init__(
        ...
        super(LoRA_mobilesam_qkv, self).__init__()
        self.qkv = qkv
...
class LoRA_sam(nn.Module):
    ...
    def __init__(self, sam_model: Sam, rank: int, lora_layer=None):
        ...
        for t_layer_i, blk in enumerate(sam_model.image_encoder.blocks):
            ...
            w_a_linear_k = nn.Linear(self.d_model, self.rank, bias=False)
            w_b_linear_k = nn.Linear(self.rank, self.d_model, bias=False)
            ...
            self.A_weights.append(w_a_linear_k)
            self.B_weights.append(w_b_linear_k)
...
class LoRA_mobilesam(nn.Module):
    ...
    def __init__(self, sam_model: Sam, rank: int, lora_layer=None):
        ...
        for t_layer_i, blk in enumerate(sam_model.image_encoder.layers[1:]):
            # if only lora on few layers
            # if t_layer_i not in self.lora_layer:
            #     continue
            for blk_i in blk.blocks:
                if isinstance(blk_i, TinyViTBlock):
                    w_qkv_linear = blk_i.attn.qkv
                    self.d_model = w_qkv_linear.in_features
                    ...
                    blk_i.attn.qkv = LoRA_mobilesam_qkv(
                        w_qkv_linear,
                        w_a_linear_q,
                        w_b_linear_q,
                        w_a_linear_k,
