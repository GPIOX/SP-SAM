...
class TinyViT(nn.Module):
    ...
    def forward_features(self, x: torch.Tensor, adapter: torch.nn.Module):
        # x: (N, C, H, W)
        ...
        for i in range(start_i, len(self.layers)):
            layer = self.layers[i]
            x = layer(x) + adapter[str(i)](x) if str(i) in adapter.keys() else layer(x)
        B,N,C=x.size()
    ...
    def forward(self, x, adapter):
        x = self.forward_features(x, adapter)
        #x = self.norm_head(x)
        #x = self.head(x)
        return x
